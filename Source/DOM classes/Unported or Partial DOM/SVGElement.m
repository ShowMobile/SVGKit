...
-(void) reCalculateAndSetViewportElementReferenceUsingFirstSVGAncestor:(SVGElement*) firstAncestor
{
	...
	{
		SVGDLog(@"[%@] WARNING: setting self (tag = %@) to be a viewport", [self class], self.tagName );
...
-(void)setParentNode:(Node *)newParent
{
	...
	else
	{
		...
		else
		{
			...
			SVGDLog(@"viewport Element = %@ ... for node/element = %@", self.viewportElement, self.tagName);
#endif
		}
	}
}
...
- (void)postProcessAttributesAddingErrorsTo:(SVGKParseResult *)parseResult  {
	...
	if( [[self getAttribute:@"transform"] length] > 0 )
	{
		...
		SVGDLog(@"[%@] WARNING: the transform attribute requires OS X 10.7 or above (we need Regular Expressions! Apple was slow to add them :( ). Ignoring TRANSFORMs in SVG!", [self class] );
#else
		NSError* error = nil;
		 ...
		 ^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop)
		{
			...
			//EXTREME DEBUG: SVGDLog(@"[%@] DEBUG: found a transform element (should be command + open bracket + args + close bracket) = %@", [self class], transformString);
			
			NSRange loc = [transformString rangeOfString:@"("];
			if( loc.length == 0 )
			{
				SVGLog(@"[%@] ERROR: input file is illegal, has an item in the SVG transform attribute which has no open-bracket. Item = %@, transform attribute value = %@", [self class], transformString, value );
				return;
			}
			...
			//EXTREME DEBUG: SVGDLog(@"[%@] DEBUG: found parameters = %@", [self class], parameterStrings);
			...
			else if( [command isEqualToString:@"rotate"] )
			{
				...
				else if( [parameterStrings count] == 3)
				{
					...
					{
					SVGLog(@"[%@] ERROR: input file is illegal, has an SVG matrix transform attribute without the required 1 or 3 parameters. Item = %@, transform attribute value = %@", [self class], transformString, value );
					return;
				}
			}
			else if( [command isEqualToString:@"skewX"] )
			{
				SVGLog(@"[%@] ERROR: skew is unsupported: %@", [self class], command );
		...
		//DEBUG: SVGDLog(@"[%@] Set local / relative transform = (%2.2f, %2.2f // %2.2f, %2.2f) + (%2.2f, %2.2f translate)", [self class], selfTransformable.transform.a, selfTransformable.transform.b, selfTransformable.transform.c, selfTransformable.transform.d, selfTransformable.transform.tx, selfTransformable.transform.ty );
#endif
		}
	}
